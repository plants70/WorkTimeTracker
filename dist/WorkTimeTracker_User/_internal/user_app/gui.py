...
class EmployeeApp(QWidget):
    ...
    def finish_shift(self):
        ...
        if reply == QMessageBox.Yes:
            try:
                # Завершаем предыдущий статус перед выходом (auto_sync отправит его позже)
                prev_id = self.db.finish_last_status(self.email, self.session_id)
                if prev_id:
                    logger.debug(f"Предыдущий статус с id={prev_id} завершен и будет синхронизирован позже.")

                result = self._log_shift_end("Завершение смены (нормальное)", reason="user")
                if result:
    ...
    def force_logout_by_admin(self):
        ...
        try:
            # --- КЛЮЧЕВОЕ ИЗМЕНЕНИЕ: Передаем self.group ---
            self._log_shift_end("Разлогинен администратором", reason="admin", group=self.group)

            # Обновим ActiveSessions
            try:
                logout_time = datetime.now().isoformat()
                if hasattr(sheets_api, "kick_active_session"):
                    sheets_api.kick_active_session(self.email, self.session_id, logout_time)
                else:
                    sheets_api.finish_active_session(self.email, self.session_id, logout_time)
            except Exception as e:
                logger.error(f"Ошибка отметки сессии в ActiveSessions: {e}")
            # ---
        except Exception as e:
    ...
    def _log_shift_end(self, comment: str, reason: str = "user", group: Optional[str] = None) -> bool:
        """
        ...
        try:
            ...
            prev_id = self.db.finish_last_status(self.email, self.session_id)
            if prev_id:
                # finish_last_status уже установил status_end_time = now(), дополнительное обновление не нужно
                # Запускаем асинхронную отправку предыдущего статуса
                threading.Thread(target=self._send_action_to_sheets, args=(prev_id,), daemon=True).start()

            now = datetime.now().isoformat()
            # --- КЛЮЧЕВОЕ ИЗМЕНЕНИЕ: immediate_sync=False для LOGOUT ---
            record_id = self.db.log_action(
                email=self.email,
                name=self.name,
                status="Завершено",
                action_type="LOGOUT",
                comment=comment,
                immediate_sync=False,  # не помечаем synced до успешной отправки
                session_id=self.session_id,
                status_start_time=now,
                status_end_time=now,
                reason=reason,
                user_group=group or self.group  # <--- Не забудьте передать группу
            )
            # ---

            # Отправляем LOGOUT в фоне
    ...
    def closeEvent(self, event):
        ...
        if reply == QMessageBox.Yes:
            ...
            try:
                ...
                self._log_shift_end("Приложение закрыто через крестик", reason="user")
                sheets_api.finish_active_session(self.email, self.session_id, datetime.now().isoformat())
            except Exception as e:
