...
class SyncQueue:
    ...
    def add_actions(self, actions: List[Dict]):
        ...
        with self.lock:
            for action in actions:
                ...
                logger.info(
                    f"Добавлено действие в очередь: id={action_id}, action_type={action['action_type']}, email={action['email']}"
                )
            self._save_queue()
    ...
    def get_pending_actions(self, limit: int = 50) -> List[Dict]:
        ...
        with self.lock:
            now = datetime.now()
            ready_actions = [
                a for a in self.queue if datetime.fromisoformat(a["next_retry"]) <= now
            ]

            # Сортируем по приоритету (по убыванию) и времени создания (по возрастанию)
            sorted_actions = sorted(
                ready_actions, key=lambda x: (-x["priority"], x["timestamp"])
            )
            logger.debug(
                f"Получено {len(sorted_actions[:limit])} готовых к отправке действий (limit={limit})"
            )
        ...
        with self.lock:
            ...
            for action in self.queue[:]:
                if action["id"] in action_ids:
                    ...
                    action["attempts"].append({"time": now, "success": success})

                    if success:
                        # Удаляем успешные действия из очереди
                        self.queue.remove(action)
                        logger.info(
                            f"Удалено успешно синхронизированное действие id={action['id']}"
                        )
                    else:
                        ...
                        logger.info(
                            f"Отмечено неудачное действие id={action['id']}, retry_count={action['retry_count']}"
                        )
            self._save_queue()
    ...
    def _calculate_next_retry(self, retry_count: int) -> datetime:
        ...
        logger.debug(
            f"Расчет времени следующей попытки: retry_count={retry_count}, delay={base_delay}s, next_retry={next_retry_time.isoformat()}"
        )
